  ...
  const fetchWins = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/getAllplanecrash?page=${page}&limit=${pageSize}&search=${search}`
      );
      ...
      if (response.success) {
        ...
        setWins([]);
        setNoData(true);
        toast.error(response.message || "Failed to fetch crash percentages");
      }
    } catch (error) {
      console.error("Error fetching crash percentages:", error);
      setWins([]);
      setNoData(true);
      toast.error("Failed to fetch crash percentages");
    } finally {
      setLoader(false);
    }
  };
    ...
    <div className="relative">
      ...
      <div className="flex justify-end">
        <button
          ...
          disabled={page === 1}
          className={`bg-blue-800 text-white p-2 m-5 text-sm rounded-lg ${page === 1 ? "opacity-50" : ""
            }`}
        >
          Previous
        </button>
        <button
          ...
          className={`bg-blue-800 text-white p-2 m-5 text-sm rounded-lg ${wins.length < pageSize || startIndex + pageSize >= count ? "opacity-50" : ""
            }`}
        >
          Next
        </button>
