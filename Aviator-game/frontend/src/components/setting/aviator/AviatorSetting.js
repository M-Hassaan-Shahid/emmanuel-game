  ...
  const fetchOldData = async () => {
    try {
      ...
      if (response.success && response.result && response.result.length > 0) {
        ...
        setData({
          gameStatus: settings.gameStatus || 0,
          withdrawalFee: settings.withdrawalFee || 0,
          minBetAmount: settings.minBetAmount || 0,
          maxBetAmount: settings.maxBetAmount || 0,
          initialBonus: settings.initialBonus || 0,
          minRecharge: settings.minRecharge || 0,
          startGameRangeTimer: settings.startGameRangeTimer || 0,
          endGameRangeTimer: settings.endGameRangeTimer || 0,
          gameStartTime: settings.gameStartTime ? settings.gameStartTime.split("T")[0] : "",
          gameBetweenEndTime: settings.gameBetweenEndTime || "",
          level1Commission: settings.level1Commission || 0,
          level2Commission: settings.level2Commission || 0,
          level3Commission: settings.level3Commission || 0,
        });
      } else {
